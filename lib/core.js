...
class NewmanConfig {
  ...
  async loopRun(root_json_file, sync = false) {
    ...
    if (sync) {
      ...
      // Run collections in parallel and wait for all to complete
      const promises = run_list.map(value => this.runCollectionAsync(value.collection, value.environment));
  ...
  async runCollectionAsync(collection, environment = undefined) {
    return new Promise((resolve) => {
      ...
      newman.run(this.getNewmanRunOptions(resolvedCollection, environment, file_name), (err, summary) => {
