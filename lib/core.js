...
class NewmanConfig {
    ...
    constructor(reporters, options = {}) {
      ...
      this.reporters_list = reporters || ['cli', 'json', 'html', 'allure', 'htmlextra', 'junit']
      ...
      this.newman_junit_report_path = './reports/junit/'
    ...
    // Validate feed file structure
    validateFeedFile(feedData, filePath) {
      const errors = [];

      if (!feedData || typeof feedData !== 'object') {
        errors.push('Feed file must be a valid JSON object');
        return errors;
      }

      if (!feedData.runs) {
        errors.push('Feed file must contain a "runs" array');
        return errors;
      }

      if (!Array.isArray(feedData.runs)) {
        errors.push('"runs" must be an array');
        return errors;
      }

      if (feedData.runs.length === 0) {
        errors.push('"runs" array cannot be empty');
        return errors;
      }

      feedData.runs.forEach((run, index) => {
        const runPrefix = `runs[${index}]`;

        if (!run || typeof run !== 'object') {
          errors.push(`${runPrefix}: Each run must be an object`);
          return;
        }

        if (!run.collection) {
          errors.push(`${runPrefix}: "collection" is required`);
        } else if (typeof run.collection !== 'string') {
          errors.push(`${runPrefix}: "collection" must be a string`);
        }

        // Validate optional string fields
        const stringFields = ['environment', 'iterationData', 'globals', 'folder'];
        stringFields.forEach(field => {
          if (run[field] !== undefined && typeof run[field] !== 'string') {
            errors.push(`${runPrefix}: "${field}" must be a string`);
          }
        });

        // Validate optional number fields
        const numberFields = ['timeout', 'timeoutRequest', 'timeoutScript', 'delayRequest'];
        numberFields.forEach(field => {
          if (run[field] !== undefined && typeof run[field] !== 'number') {
            errors.push(`${runPrefix}: "${field}" must be a number`);
          }
        });

        // Validate optional boolean fields
        if (run.bail !== undefined && typeof run.bail !== 'boolean') {
          errors.push(`${runPrefix}: "bail" must be a boolean`);
        }
      });
  ...
  async loopRun(root_json_file, sync = false) {
    ...
    try {
      root_file = require(root_json);
    } catch (e) {
      console.error(`Error loading feed file: ${e.message}`);
      this.results.errors.push({ feedFile: root_json_file, error: e.message });
      return this.results;
    }

    // Validate feed file structure
    const validationErrors = this.validateFeedFile(root_file, root_json_file);
    if (validationErrors.length > 0) {
      console.error('Feed file validation failed:');
      validationErrors.forEach(err => console.error(`  - ${err}`));
      this.results.errors.push({ feedFile: root_json_file, validationErrors });
      return this.results;
    }
  ...
  runCollection(collection, environment = undefined) {
	...
	getNewmanRunOptions(collection, environment, file_name, options = {}) {
		...
		let opt = {
			...
			reporter: {
				...
				junit: {
					export: this.newman_junit_report_path + reportFileName + '.xml'
				}
			},
			insecure: true,
		};
    ...
    async clearResultsFolder() {
        ...
        await this.removeDirectory(files.getCurrentDirectoryBase() + this.newman_junit_report_path);
    }
